...
public class WorkflowController {
    ...
    public ResponseEntity<Map<String, String>> handleMessage_old(@RequestBody MessagePayload messagePayload) {
        ...
        logger.info("Received message with threadId: {} (channelId: {}, userId: {}, content: '{}')", threadRootId, channelId, userId, message);
...
//            List<ActiveTask> existingTask = activeTaskRepository.findByChannelIdAndUserId(channelId, userId);
            ...
            if (!existingTask.isEmpty()) {
                ...
                logger.info("Found active task for channelId: {}, workflowId: {}, threadId: {}, treating message as user response", channelId, workflowId, threadRootId);
...
//                activeTaskRepository.save(activeTask);
                ...
                logger.info("No active task found for channelId: {}, threadId: {}, extracting goal from message", channelId, threadRootId);
                ...
                if (extractedGoal == null || extractedGoal.getGoal() == null || extractedGoal.getGoal().trim().isEmpty()) {
                    logger.warn("Could not extract valid goal from message: '{}' (threadId: {})", message, threadRootId);
                    Map<String, String> response = new HashMap<>();
                ...
                logger.info("Extracted goal: '{}' from message (threadId: {})", extractedGoal.getGoal(), threadRootId);
                ...
                if (workflowResponse.getStatusCode() == HttpStatus.OK) {
                    ...
                    logger.info("Created new active task record for channelId: {}, workflowId: {}, threadId: {}", channelId, workflowId, threadRootId);
                }
